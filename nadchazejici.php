...
<div class="bodywrapper">
    ...
    <section class="content">
	    ...
	    <?php
	        ...
	        echo "\r\n";
	        echo '<!-- auxiliary $cups upcoming type and count for debug purposes -->';
	        echo "\r\n";
        ...
        <?php
            if(count($cups)!=0) {
	            ...
	            <?php foreach ($cups as $cup): ?>
                    <article class="zavod" onclick="location.href='zavod.php?id=<?=$cup->id?>';">
                            <h1><?= h($cup->name) ?></h1> <h2 style="text-align:center"><?=  date('j. m. Y', strtotime($cup->time_start)) ?> – <?=  date('j. m. Y', strtotime($cup->time_end)) ?></h2> <p><?= h($cup->description)?></p>
                    </article>
	            <?php endforeach; ?>
	            <?php
            }
            ...
            {
	            echo "<p style=\"text-align:center;\">–<span style=\"border: 1px solid black;\">&nbsp;0 NADCHÁZEJÍCÍCH ZÁVODŮ&nbsp;</span>–</p>";
            }
        ?>
    </section>
